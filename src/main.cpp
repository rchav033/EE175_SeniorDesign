...
int16_t GyroX=Wire.read()<<8 | Wire1.read();
int16_t GyroY=Wire.read()<<8 | Wire1.read();
int16_t GyroZ=Wire.read()<<8 | Wire1.read();
...
battery_voltage();
if (Voltage > 7.0) { digitalWrite(19, LOW); BatteryAtStart=BatteryDefault; }
else if (Voltage < 6.8) {BatteryAtStart=30/100*BatteryDefault ;}
...
analogWrite(3,MotorInput1);
analogWrite(4,MotorInput2);
analogWrite(5,MotorInput3);
analogWrite(6,MotorInput4);
battery_voltage();
